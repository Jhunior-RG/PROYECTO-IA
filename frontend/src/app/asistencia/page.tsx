...
export default function Asistencia() {
  ...
  useEffect(() => {
    async function init() {
      ...
      for (const persona of personas) {
        ...
        if (descs.length) {
          descriptors.push(
            new faceapi.LabeledFaceDescriptors(persona.nombre, descs)
          );
        }
      }
  ...
  const handleVideoPlay = () => {
    ...
    setInterval(async () => {
      ...
      for (const d of resized) {
        ...
        const label = match.label === "unknown" ? "Desconocido" : match.label;
        const texto = `${label} `;
        ...
        if (nombre !== "unknown") {
          if (!personasActivas[nombre]) {
            ...
            console.log("actualizando ultima vez visto de ", nombre);
  ...
  return (
    <div className="p-6 flex flex-col items-center justify-center min-h-screen bg-gray-50 space-y-4">
      <h1 className="text-2xl font-bold mb-4">Asistencia en {selectedAula}</h1>
      <p className="text-sm text-gray-600 max-w-3xl mb-4 text-center">
        ...
        permisos para acceder a ella. El reconocimiento facial se activará
        automáticamente al detectar rostros.
      </p>
      ...
      <div className="text-sm text-gray-500 text-center space-y-2">
        {Object.keys(personasActivas).length > 0 && (
          <div className="bg-gray-100 rounded-lg p-3 max-w-md mx-auto">
            <p className="text-xs font-semibold mb-2">Personas detectadas:</p>
